...
class Compilation {
  ...
  buildModule(name, modulePath) {
    ...
    let loaders = [];
    rules.forEach(rule => {
      // 如果正则匹配上了，则把此 rule 对应的 loader 添加到 loaders 数组里
    ...
    // "./src/title.js" 每个模块都有一个 ID，id: './src/entry1.js'
    // 模块ID就是相对于项目根目录的相对路径
    let moduleId = "./" + path.relative(baseDir, modulePath)
    // 创建一个模块对象， moduleId 是相对于项目根目录的相对路径，dependencies 表示此模块依赖的模块
    // names 表示此模块添加了几个入口依赖了，入口的名称 [entry1, entry2]
    ...
    traverse(ast, {
      CallExpression: ({ node }) => {
        if (node.callee.name === 'require') {
          ...
          // 获取依赖模块的 ID，也就是相对于根目录的相对路径
          let depModuleId = './' + path.relative(baseDir, depModulePath);
          // 修改 AST 语法树，把 require 方法的参数变成这依赖的模块ID
